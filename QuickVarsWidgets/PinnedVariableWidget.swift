...
/// opens the app via `quickvars://reveal` and still goes through the normal
...
struct PinnedVariableWidgetView: View {
    ...
    var body: some View {
        content
            .widgetURL(entry.pinnedItemID.flatMap { URL(string: "quickvars://reveal?id=\($0.uuidString)") })
...
struct PinnedVariableWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("Pinned QuickVar")
        .description("A Lock Screen shortcut to one QuickVar you've pinned. Never shows its name or value — Face ID is still required to reveal it.")
