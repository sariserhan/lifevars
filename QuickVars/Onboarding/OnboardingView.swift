...
/// SPEC.md §1 — five screens: what QuickVars is, what you'd store, how
/// retrieval actually works (voice included), where it lives (local +
/// encrypted), then the Face ID enable action. The "first QuickVar" prompt
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("Stays on your phone.\nNowhere else.")
                .font(.title.bold())
                .multilineTextAlignment(.center)
            Text("Every QuickVar is encrypted the moment you save it. No account, no server, no cloud — this device is the only place any of it ever exists.")
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 40)
            Text("That cuts both ways: losing this device means losing your QuickVars — unless you've made your own encrypted, password-protected backup (Settings → Backup).")
                .font(.caption)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 40)
