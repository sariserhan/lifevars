...
The Xcode project is generated from `project.yml` via [XcodeGen](https://github.com/yonaskolb/XcodeGen) — `LifeVars.xcodeproj` is not hand-edited and not something you should open Xcode's project editor to change. Edit `project.yml` (or add/remove files under `LifeVars/`), then regenerate:
...
Do this after adding or removing any Swift file — XcodeGen globs `LifeVars/` for sources, so new files need a regenerate to show up in the project (Xcode will auto-detect the change if it's already open).

### Build & run

```sh
xcodebuild -project LifeVars.xcodeproj -scheme LifeVars \
  -destination 'platform=iOS Simulator,name=iPhone 17' -configuration Debug build
```

Or just open `LifeVars.xcodeproj` in Xcode and hit Run.
...
- Siri does not function in the Simulator at all, on any app — the `FindLifeVarIntent` build/registration is verified, but invoking it via Siri needs a real device.
- StoreKit purchases need an Xcode Run-button (or `xcodebuild test`) launch — `LifeVars.storekit` is wired into the scheme, but `simctl launch` bypasses Xcode's scheme-driven launch entirely, so the local config never attaches.
...
```
LifeVars/
├── App/          LifeVarsApp (entry point, scenePhase → lock + appearance wiring), RootView, AppSwitcherOverlay (SPEC.md §2.3)
...
├── Intents/       FindLifeVarIntent + AppShortcuts — never returns a value to Siri (SPEC.md §13)
├── Settings/      SettingsView, PaywallView, Privacy/Security info screens (SPEC.md §11)
├── Store/         StoreManager — StoreKit 2 non-consumable
├── Data/          Persistence (SwiftData, excluded from backup per §15.2), LifeVarStore (encrypt/decrypt CRUD)
├── Models/        LifeVar, LifeVarMetadata, Category, Classification (§9.1), Matcher (§9.2)
