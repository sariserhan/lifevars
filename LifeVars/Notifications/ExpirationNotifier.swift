...
/// unlock instead (LifeVarStore.reload), no reminder needed for those.
///
...
enum ExpirationNotifier {
    ...
    static func scheduleReminder(for id: UUID, expiresAt: Date) {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound]) { granted, _ in
            ...
            content.title = "LifeVars"
            content.body = "Something you saved is expiring soon — open LifeVars to check."
