...
/// §1.4 first-LifeVar prompt, `prefillName` skips straight to step 2 with
...
struct AddVariableView: View {
    ...
    private var valueStep: some View {
        Form {
            ...
            Section {
                ...
                if isPinned {
                    Text("Adds a shortcut widget to your Lock Screen. Only one LifeVar can be pinned — pinning this replaces any current pin. The widget itself never shows the name or value, only a generic locked icon; Face ID is still required to reveal it.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func save() {
        ...
        guard !store.nameExists(trimmedName) else {
            errorMessage = "You already have a LifeVar named this."
            return
        }
