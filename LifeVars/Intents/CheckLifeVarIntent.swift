...
/// The headless counterpart to FindLifeVarIntent — this one authenticates
...
/// decrypted value to Siri" (FindLifeVarIntent's rule, SPEC.md §13): the
/// trade for skipping the app-open step is that the result briefly exists as
/// system Siri UI, whose lifecycle LifeVars doesn't control the way it
...
struct CheckLifeVarIntent: AppIntent {
    static var title: LocalizedStringResource = "Check a LifeVar"
    static var description = IntentDescription("Check something you saved in LifeVars with Face ID, without opening the app.")
    ...
    func perform() async throws -> some IntentResult & ShowsSnippetView {
        ...
        context.localizedReason = "Check a LifeVar"
        guard let dek = try? KeychainDEKStore().fetchOrCreateDEK(context: context) else {
            return .result(view: LifeVarSnippetView(state: .authFailed))
        }

        let modelContext = ModelContext(Persistence.makeContainer())
        switch LifeVarLookup.lookup(query: query, dek: dek, modelContext: modelContext) {
        case .notFound:
            return .result(view: LifeVarSnippetView(state: .notFound))
        case .ambiguous:
            return .result(view: LifeVarSnippetView(state: .ambiguous))
        case .match(let record, let metadata):
            ...
            return .result(view: LifeVarSnippetView(state: .revealed(name: metadata.name, value: formatted)))
        }
    }
}
