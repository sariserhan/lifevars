...
@main
struct LifeVarsApp: App {
    @StateObject private var session: SessionManager
    @StateObject private var store: LifeVarStore
    ...
    init() {
        ...
        let lifeVarStore = LifeVarStore(modelContext: container.mainContext, session: sessionManager)
        _session = StateObject(wrappedValue: sessionManager)
        _store = StateObject(wrappedValue: lifeVarStore)
        ...
        watchSession = PhoneConnectivitySession(session: sessionManager, store: lifeVarStore)
    }
    ...
    var body: some Scene {
        WindowGroup {
            ...
            .onOpenURL { url in
                guard url.scheme == "lifevars", url.host == "reveal",
