...
/// Adds every item in the backup as new — see LifeVarStore.importBackupItems().
        ...
        NavigationStack {
            Form {
                if let restoredCount {
                    Section {
                        Text("Restored \(restoredCount) LifeVar\(restoredCount == 1 ? "" : "s").")
                    ...
                    Section {
                        ...
                        Text("Pick a file created with LifeVars' Export Encrypted Backup.")
                    }
                } else {
                    Section {
                        SecureField("Backup password", text: $password)
                    } footer: {
                        Text("Restoring adds these as new LifeVars — it won't overwrite or remove anything already here.")
                    }
    ...
    private func restore() {
        ...
        Task {
            do {
                ...
                errorMessage = "Wrong password, or that file isn't a valid LifeVars backup."
            }
            isRestoring = false
        }
    }
