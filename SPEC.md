...
`isPinned`'s corresponding item *id* and *category* (never its name or value) are also republished in the clear to a shared App Group store for the Lock Screen widget to read (§19.4) — the id, for the same "safe to store in the clear" reasoning as above; the category, a separate, deliberate call weighed directly against showing the actual name (§19.4 explains the trade-off).
